...
Intune の Configuration Policy の Assignment について、
**User Group と Device Group の違い**を以下のとおりご説明いたします。

### 1. User Group に割り当てる場合

ポリシーの対象は「ユーザー」となります。

* 対象ユーザーが Intune 管理端末にサインインした際に、ポリシーが適用されます。
* ユーザーが複数の端末を利用する場合も、基本的にはそのユーザーに対してポリシーが適用されます。
* ユーザーごとに設定を変更したい場合に適しています。

例：

* Office / OneDrive のユーザー設定
* ユーザー証明書
* ユーザー単位で適用する設定

### 2. Device Group に割り当てる場合

ポリシーの対象は「端末」となります。

* 端末を使用するユーザーに関係なく、対象端末にポリシーが適用されます。
* 別のユーザーが同じ端末にサインインした場合でも、端末に対する設定は引き続き適用されます。
* 端末単位で設定を統一したい場合に適しています。
...
* **User Group：ユーザーを基準にポリシーを適用する**
* **Device Group：端末を基準にポリシーを適用する**

そのため、

「特定のユーザーに設定を適用したい」場合は **User Group**、
「使用するユーザーに関係なく、特定の端末に設定を適用したい」場合は **Device Group**

を使用するのが基本的な考え方となります。

なお、実際の適用範囲については、Configuration Policy 内の各設定が User Scope / Device Scope のどちらに該当するかによっても動作が異なるため、設定内容と合わせて確認する必要があります。
