...
then recreate and verify them on a target IAM group in another account.
...
def canonicalize_policy_value(value: Any) -> Any:
    """Canonicalize policy JSON for order-insensitive statement comparison.

    IAM treats arrays such as Action, Resource, and condition values as sets for
    authorization purposes, so sorting them prevents harmless ordering
    differences from appearing as mismatches.
    """
    if isinstance(value, dict):
        return {
            key: canonicalize_policy_value(value[key])
...
def statement_key(statement: Dict[str, Any]) -> str:
    return compact_json(canonicalize_policy_value(statement))


def statement_index(statements: Iterable[Dict[str, Any]]) -> Dict[str, Dict[str, Any]]:
    """Return unique canonical statements keyed by stable compact JSON."""
    indexed: Dict[str, Dict[str, Any]] = {}
    for statement in statements:
        normalized = canonicalize_policy_value(statement)
        indexed[compact_json(normalized)] = normalized
    return indexed


def get_group_policy_documents(iam: Any, group_name: str) -> List[Dict[str, Any]]:
    """Read every managed and inline policy currently applied to an IAM group."""
    iam.get_group(GroupName=group_name, MaxItems=1)
    ...
    for attached in paginate(
        ...
        document, version_id, managed_type = get_managed_policy_document(
            iam, attached["PolicyArn"]
        )
        documents.append(
            {
                "origin": "managed",
...
def verify_permissions(
    ...
    """Compare the expected source permission union with the live target group."""
    data = json.loads(input_file.read_text(encoding="utf-8"))
    if data.get("schema_version") != 1:
        raise MigrationError("Unsupported export schema version")
    ...
    for policy in data.get("policies", []):
        document = normalize_policy_document(policy["document"])
        if rewrite_ids:
            document = rewrite_account_id(
                document,
                source_account_id,
                target_account_id,
            )
        expected_statements.extend(extract_statements(document))

    target_policies = get_group_policy_documents(iam, target_group)
    actual_statements: List[Dict[str, Any]] = []
    for policy in target_policies:
        actual_statements.extend(extract_statements(policy["document"]))

    expected = statement_index(expected_statements)
    actual = statement_index(actual_statements)
    ...
    report = {
        ...
        "account_id_rewrite": rewrite_ids,
        "expected_unique_statement_count": len(expected),
        "actual_unique_statement_count": len(actual),
        "missing_statement_count": len(missing_keys),
        "extra_statement_count": len(extra_keys),
        "target_policies": [
            {
                "origin": item["origin"],
        ...
        "missing_statements": [expected[key] for key in missing_keys],
        "extra_statements": [actual[key] for key in extra_keys],
    }
    ...
    print(f"Target policies found: {len(target_policies)}")
    print(f"Expected unique statements: {len(expected)}")
    print(f"Actual unique statements: {len(actual)}")
    print(f"Missing statements: {len(missing_keys)}")
    print(f"Extra statements: {len(extra_keys)}")
    ...
    if missing_keys or extra_keys:
        if missing_keys:
            print("\nFirst missing statement:")
            print(json.dumps(expected[missing_keys[0]], ensure_ascii=False, indent=2))
        if extra_keys:
            print("\nFirst extra statement:")
            print(json.dumps(actual[extra_keys[0]], ensure_ascii=False, indent=2))
        raise MigrationError(
            "Verification failed: the target group policy statements do not "
            "exactly match the expected cumulative source statements."
        )

    print("VERIFIED: target group statements exactly match the expected source union.")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    verify_parser = subparsers.add_parser(
        "verify",
        help="Verify that a target IAM group exactly matches an exported permission union",
    )
    verify_parser.add_argument("--target-profile", default="pck_stg")
    verify_parser.add_argument("--input", type=Path, required=True)
    verify_parser.add_argument("--target-group", default="ap-ai")
    verify_parser.add_argument(
        "--rewrite-account-id",
        action="store_true",
        help="Apply the same source-to-target account ID replacement used during migration.",
    )
    verify_parser.add_argument(
        "--report",
        type=Path,
        default=Path("ap-ai-verification-report.json"),
        help="Write missing/extra statement details to this JSON report.",
    )
